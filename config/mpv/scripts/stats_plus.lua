...
local function no_ASS(t)
	...
	elseif not o.persistent_overlay then
		...
		return tostring(t):gsub("\\", ESC_BACKSLASH):gsub("{", "\\{"):gsub("^ ", "\\h")
	end
end
...
local function ellipsis(s, maxlen)
	if not maxlen or s:len() <= maxlen then
		return s
	end
...
local function cmd_subject(cmd)
	...
	sname = tok == "script_message_to" and cmd:match(TOKEN) or tok == "script_binding" and cmd:match(TOKEN .. "/")
...
local function get_kbinfo_lines(width)
	...
	for _, bind in pairs(bindings) do
		if
			bind.priority >= 0
			and (not active[bind.key] or (active[bind.key].is_weak and not bind.is_weak) or (bind.is_weak == active[bind.key].is_weak and bind.priority > active[bind.key].priority))
	...
	local spre = term and kspaces .. "   "
		or format("{\\q2\\fn%s}%s   {\\fn%s}{\\fs%d\\u1}", o.font_mono, kspaces, o.font, 1.3 * o.font_size)
	...
	for _, bind in ipairs(ordered) do
		...
		append(info_lines, ellipsis(bind.cmd, cutoff), { prefix = kpre .. no_ASS(align_right(bind.key)) .. kpost })
	end
	return info_lines
end
...
local function add_file(s)
	...
	if ed_cond then
		append_property(s, "edition-list/" .. tostring(edition) .. "/title", { prefix = "版本：" })
...
local function add_video(s)
	...
	if
		append_property(s, compat("decoder-frame-drop-count"), { prefix = "丢帧暂计：", suffix = "（解码）" })
	then
		append_property(s, compat("frame-drop-count"), { suffix = "（输出）", nl = "", indent = "" })
	end
	if append_property(s, "display-fps", { prefix = "显示刷新：", suffix = "（指定）" }) then
		append_property(s, "estimated-display-fps", { suffix = "（实测）", nl = "", indent = "" })
	else
		append_property(s, "estimated-display-fps", { prefix = "显示刷新率：", suffix = "（实测）" })
	end
	if append_property(s, compat("container-fps"), { prefix = "容器帧率：", suffix = "（指定）" }) then
		append_property(s, "estimated-vf-fps", { suffix = "（实测）", nl = "", indent = "" })
	else
		append_property(s, "estimated-vf-fps", { prefix = "滤镜链帧率：", suffix = "（实测）" })
	end
...
local function keybinding_info(after_scroll)
	...
	return table.concat(header) .. table.concat(kbinfo_lines, "", page.offset, maxline)
end
...
local function cache_stats()
	...
	if not display_timer.oneshot and o.use_ass then
		r_graph =
			generate_graph(cache_ahead_buf, cache_ahead_buf.pos, cache_ahead_buf.len, cache_ahead_buf.max, nil, 0.8, 1)
	...
	if not display_timer.oneshot and o.use_ass then
		speed_graph =
			generate_graph(cache_speed_buf, cache_speed_buf.pos, cache_speed_buf.len, cache_speed_buf.max, nil, 0.8, 1)
	...
	append(stats, utils.format_bytes_humanized(info["total-bytes"]), { prefix = "总计大小：" })
	append(stats, utils.format_bytes_humanized(info["fw-bytes"]), { prefix = "预计大小：" })
	...
	append(stats, info["bof-cached"] and "是" or "否", { prefix = "起点已缓冲：" })
	append(stats, info["eof-cached"] and "是" or "否", { prefix = "终点已缓冲：" })

	local ranges = info["seekable-ranges"] or {}
	for n, r in ipairs(ranges) do
		append(
			stats,
			mp.format_time(r["start"]) .. " - " .. mp.format_time(r["end"]),
...
local function print_page(page, after_scroll)
	...
	if o.persistent_overlay then
		...
		mp.osd_message(
			(o.use_ass and ass_start or "") .. ass_content,
...
mp.add_key_binding(nil, "display-stats", function()
	process_key_binding(true)
end, { repeatable = true })

-- Toggling key binding
mp.add_key_binding(nil, "display-stats-toggle", function()
