...
function M.open(cwd)
  ...
  if vim.fn.bufexists(M.buffer) == 0 then
    ...
    M.job_id = vim.fn.termopen(vim.o.shell, {
      ...
      detach = true,
      on_exit = function()
        if M.job_id == job_id then M.destroy() end
      end,
    })
    job_id = M.job_id
  else
...
function M.destroy()
  if vim.fn.win_gotoid(M.window) ~= 0 then M.height = vim.api.nvim_win_get_height(M.window) end
  if vim.fn.bufexists(M.buffer) == 1 then vim.api.nvim_buf_delete(M.buffer, { force = true }) end
