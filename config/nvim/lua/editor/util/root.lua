...
function M.detectors.lsp(buf)
    ...
    local clients = Editor.lsp.get_clients { bufnr = buf }
...
function M.info()
    ...
    lines[#lines + 1] = "```"
    Editor.info(lines, { title = "LazyVim Roots" })
...
function M.get(opts)
    ...
    return Editor.is_win() and ret:gsub("/", "\\") or ret
end
