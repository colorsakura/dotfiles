vim.api.nvim_create_augroup('config', { clear = false })
		...
		config = function()
			...
			do
				...
				vim.api.nvim_create_autocmd('User',
					{ command = command, group = 'config', pattern = 'GitSignsUpdate' })
			end

			--- Set buffer variables for file icon and color.
			--- @return {color: string, icon: string}
			local function buf_init_devicons()
				local icon, color = devicons.get_icon(vim.fn.expand '%:t', vim.fn.expand '%:e',
					{ default = true })
				local dev_icons = {
					color = vim.api.nvim_get_hl(0, { link = false, name = color }).fg,
					icon =
					    icon
				}
			...
			require('heirline').setup({
				statusline =
				{
					...
					{ -- ViMode {{{
						hl = function(self)
							vim.api.nvim_set_hl(0, self.group,
								{ bg = SIDEBAR, fg = self.color, bold = true })
							return self.group
						end,
						init = function(self)
					...
					{ -- File Info {{{
						...
						{ -- File size {{{
							init = function(self)
								self.stat = vim.loop.fs_stat(vim.api
									.nvim_buf_get_name(0))
							end,
						...
						init = function(self)
							...
							if #diagnostics < 1 then
								...
								for _, diagnostic in ipairs(diagnostics) do
									self.diagnostics[diagnostic.severity] = self
...

						{
							...
							LEFT_SEPARATOR,
							{
...

								{
									...
									provider = function(self)
										return
										    self:provide(vim.diagnostic.severity
										    .ERROR)
									end
								},
								{
