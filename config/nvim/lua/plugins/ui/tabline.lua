...
local function get_icon_and_title(winid, is_cur, title_hl)
  ...
  if vim.fn.win_gettype(winid) == "loclist" then filetype = "loclist" end
  ...
  if vim.wo[winid].diff then title = title .. " (diff)" end
...
function M.render()
  ...
  for i, tabpage in ipairs(vim.api.nvim_list_tabpages()) do
    ...
    for _, win in ipairs(wins) do
      ...
      if win == winid then cur_diag = error + warn end
    end
    ...
    if bufmodified ~= 0 then
      if total_diag > 0 then
        table.insert(items, string.format("%%#%s#%s%%#%s#", diag_hl, icons.misc.circle_filled, tab_hl))
      else
...
vim.o.showtabline = 2
vim.o.tabline = "%{%v:lua.require('plugins.ui.tabline').render()%}"
