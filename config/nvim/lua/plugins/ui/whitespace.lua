--- copy from <https://github.com/mcauley-penney/visual-whitespace.nvim>
--- 与原版相比，本插件只在 Block-Visual 模式显示空格
---
...
local function get_marks(s_pos, e_pos, mode)
  ...
  for cur_row = srow, erow do
    ...
    if mode == "v" then
      ...
      if adjusted_ecol ~= line_len then line_text = line_text:sub(1, adjusted_ecol) end
    else
      adjusted_scol = scol
    end
...
local clear_ws_hl = function() api.nvim_buf_clear_namespace(0, NS_ID, 0, -1) end
...
local function init_aucmds()
  if CFG.enabled then
    aucmd("ModeChanged", {
      group = hl_augrp,
      pattern = "*:[vV]",
      callback = function() return highlight_ws() end,
    })

    aucmd("CursorMoved", {
      group = hl_augrp,
      callback = function() return vim.schedule(highlight_ws) end,
    })

    aucmd("ModeChanged", {
      group = hl_augrp,
      pattern = "[vV]:[^vV]",
      callback = function() return clear_ws_hl() end,
    })
  else
...
M.setup = function(user_cfg)
  ...
  aucmd({ "BufEnter", "WinEnter" }, {
    ...
    callback = vim.schedule_wrap(function()
      ...
      if prev_enabled ~= CFG.enabled then init_aucmds() end
    end),
  })

  aucmd({ "ColorScheme" }, {
    group = core_augrp,
    callback = function() api.nvim_set_hl(0, "VisualNonText", CFG["highlight"]) end,
  })
